...
while len(contours) > 9:
    ...
    for ct in contours:
        if abs(medianWidth - getWidth(ct)) > furthestFromMedianWidth:
            furthestFromMedianWidth = abs(
                medianWidth - getWidth(ct))
    ...
    for ct in contours:
        ...
        if abs(medianHeight - getHeight(ct)) > furthestFromMedianHeight:
            furthestFromMedianHeight = abs(
                medianHeight - getHeight(ct))
...
# wasSuccessful = cv2.imwrite(imdir + "post.jpg", img)
